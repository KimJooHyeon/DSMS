...
function keyevent(e) {
	...
	switch(key) {
		...
		case 'done':
			...
			const showText = $('.sign-up-reg-text')
			
			if(reg.test(text)) {
				$.ajax({
					url: "/member?cmd=signUpCheck.do",
					data: {
					...
					success: function(res) {
						if(res.trim() === "true") {
							showText.html('가입 가능합니다.').css('color', 'green')
							regNoCheck = true
						} else {
							showText.html('이미 가입하셨습니다.').css('color', 'red')
						}
					},
					error: function(e) {
						console.log(e)
				...
				showText.html('주민등록 번호를 정확히 입력해 주세요.').css('color', 'red')
			}
			break;
		default :
